...
namespace app\api\service\Ueditor;
...
class Ueditor extends Controller
{
    ...
    {
        $json_fiel = dirname(\think\facade\Env::get('root_path')) . '/main/public/static/ueditor1433/php/config.json';

        $CONFIG = json_decode(preg_replace("/\/\*[\s\S]+?\*\//",
            "",
            file_get_contents($json_fiel)),
            true);
    ...
    private function upFile($fieldName)
    {
        ...
        if ($info) {//上传成功
            ...
            $data = array(
                'state' => 'SUCCESS',
                'url' => config('my_config.img_url') . $fname,
